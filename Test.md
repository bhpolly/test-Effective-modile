...
### Заголовки запроса  
Authorization: _Bearer {JWT_token}_   
...
| --- | --- | --- | --- | --- |
| 1 | companies | arrays | Да | Список компаний-партнеров |
| 2 | companyId | integer | Да | Уникальный идентификатор компаний-партнеров |
| 3 | companyName | string | Да | Название компании-партнера |
| 4 | companyLogo | string | Нет | Логотип компании-партнера |
| 5 | companyUrl | string | Да | Ссылка  для перехода на сайт компании-партнера |
| 5 | fastDelivery | boolean | Да | Возможность быстрой доставки |
| 7 | fastDeliveryTimeMin | integer | Нет | Минимальное время быстрой доставки (при ее доступности)|
| 8 | fastDeliveryTimeMax | integer | Нет | Максимальное время быстрой доставки (при ее доступности) |
| 9 | nearestDeliveryStart | string | Нет | Наиболее ранее время окна ближайшей доставки (при отсутсвии быстрой доставки)|
| 10 | nearestDeliveryEnd | string | Нет | Наиболее позднее время окна ближайшей доставки (при отсутсвии быстрой доставки)|
| 11 | message | string | Да | Причина ошибки обработки запроса |
| 12 | userMessage | string | Нет | Сообщение для пользователя в случае отсутствия доставки |
...
<img width="1771" height="881" alt="image" src="https://github.com/user-attachments/assets/86dfb0fd-e17a-451c-abd0-a091b1e1378b" />
## Описание архитектуры 
| № | Название компонента | Описание | Функции | 
| --- | --- | --- | --- |
| 1 | Mobile App (мобильный клиент) | Точка входа пользователя в систему | Просмотр каталога, управление корзиной, оформление заказа, отмена заказа|
| 2 | API Gateway | Единая точка входа в backend | Маршрутизация запросов к сервисам, авторизация / аутентификация, rate limiting, логирование, агрегация ответов (опционально)|
| 3 | User Service | Управление пользователями | Хранение профиля пользователя, хранение device tokens для push, управление авторизацией, публикация событий |
| 4 | Catalog Service | Управление товарами | Хранение каталога, поиск товаров, карточки товаров, цены и доступность, публикует события|
| 5 | Cart Service | Управление корзиной пользователя | Добавление товаров, удаление товаров, изменение количества, расчёт стоимости корзины, публикует события|
| 6 | Order Service | Управление заказами | Создание заказа, изменение статуса, отмена заказа, хранение истории заказов, интеграция с платёжными системами, публикует события|
| 7 | Notification Service | Отправка уведомлений пользователям | Формирование текста уведомления, получение device token из User Service, отправка уведомления push-provider |
| 8 | Kafka | Брокер сообщений (асинхронное взаимодействие сервисов) | Доставка событий между сервисами, гарантированная доставка сообщений |
| 9 | Платёжные сервисы (внешние) | Обработка оплаты | Проведение платежа, подтверждение/ошибка оплаты, возвраты|
| 10 | Push-provider (внешние) | Доставка push | Доставка push-уведомления на устройство пользователя |

## Требования к ПО (общие к процессу отправки push-уведомлений)
1. После установки пользователем мобильного приложения система запрашивает пользователя разрешение на получение push-уведомлений. Системный диалог разрешения запрашивается приложением только при отсутствии ранее выданного ответа пользователя.   
      1.1. В случае согласия пользователя приложение получает push device token от push-provider и передает его в User Service с помощью API. User Service сохраняет токен устройства в базе данных (User DB) с указанием: userId, platform (IOS, Android), provider, дата регистрации токена.   
      1.2. Если от пользователя был получени отказ, приложение должно отображать собственный экран с объяснением ценности уведомлений и возможностью перейти в настройки устройства. Например, “Разрешите уведомления, чтобы отслеживать заказы”.      
   
2. При возникновении события в системе (в сервисах Catalog Service, User Service, Cart Service, Order Service) публикуется сообщение в соответсующий топик брокера сообщений (Kafka).   
   2.1. Сообщение должно содержать следующую информацию: тип события, userId (если событие касается конкретного пользователя), payload уведомления, timestamp события.   
   2.2. В зависимости от типа сообщений назначаются гарантии доставки at most once  для некритичных событий (например, рекламные рассылки) или at least once для бизнес-критичных уведомлений (например, оплата заказа). В случае с гарантией доставки at least once у получателя продумана система обработки дубликатов (idempotency key).    
      
3. Notifivation Service подписан на топики и читает сообщения из брокера сообщений:     
   3.1. Notifivation Service обращается к User Service по API (```GET /users/{id}/devices```) для получения активных токенов всех устройств (device token) пользователя (пользователей), сохраненных в базе данных сервиса (User DB).   
   3.2. После получения ответа от User Service в Notification Service происходит валидация ответа по количеству полученных токенов устройств: ответ обязан содержать хотя бы один токен устройства на пользователя. В противном случае процесс отправки push-уведомления конкретному пользователю прекращается. Если событие в системе является критичным, то инициируется процесс отправки SMS-уведоммления.    
   3.3. Notifivation Service формирует уведомление и отправляет синхронный запрос с данными уведомления (токены устройств и другую информацию об уведомлении) push-provider по API. Уведомление должно формироваться и отправляться в push-provider не более чем через 5 секунд после возникновения события в системе.   
   3.4. Если событие считается некритичным, то процесс отправки push-уведомлений с 23:00 до 07:00 по локальному времени пользователя прекращается. Критичные уведомления отправляются независимо от времени.   
      
4. При получении положительного ответа "200 OK" от push-provider уведомление сообщение считается отправленным. Иначе:   
   4.1. В случае, когда от push-provider приходит ошибка "404 NOT_FOUND":   
      4.1.1. Notification Service отправляет запрос (```DELETE /users/{id}/devices/{id}```) User Service для удаления невалидного токена устройства. В случае, когда событие определено в системе как критически важное, инициируется процесс отправки SMS-сообщения.   
   4.2. В случае получения ошибки "VALIDATION_ERROR" (неправильно указаны параметры запроса при отправке сообщения):   
      4.2.1. ```providers.%provider_name%``` (ошибка в провайдере): _(необходимо продумать систему алертов для VALIDATION_ERROR, т.к. это не временная проблема а тут нужны правки в работе системы)_   
      4.2.2. ```tokens.%provider_name%``` (ошибка в токене устройства): инициируется процесс удаления невалидного токена устройства из системы (п. 4.1.1)   
      4.2.3. ```message.%field%``` (ошибка в сообщении): _(необходимо продумать систему алертов для VALIDATION_ERROR, т.к. это не временная проблема а тут нужны правки в работе системы)_   
   4.3. В случае получения ошибки "PROVIDER_ERROR" (ошибка отправки в провайдер пушей):   
      4.3.1. ```%provider_name%: %internal%```(внутренняя ошибка провайдера): повторно отправляем запрос (retry с backoff 5s, 30s, 300s). Если количество попыток превышено, но сообщение не получено провайдером, то процесс отправки уведомления прекращается. В случае, когда событие определено в системе как критически важное, инициируется процесс отправки SMS-сообщения.   
      4.3.1. ```%provider_name%: %validation error%``` (ошибка валидации на стороне провайдера): _(необходимо продумать систему алертов для VALIDATION_ERROR, т.к. это не временная проблема а тут нужны правки в работе системы)_   
      4.3.1. ```%provider_name%: %invalid auth token%``` (ошибка auth-токена): _(необходимо продумать систему алертов для VALIDATION_ERROR, т.к. это не временная проблема а тут нужны правки в работе системы)_   
      4.3.1. ```%provider_name%: %too many requests%``` (слишком много запросов): _(необходимо продумать систему алертов для VALIDATION_ERROR, т.к. это не временная проблема а тут нужны правки в работе системы)_   
      4.3.1. ```%provider_name%: %invalid tokens%``` (ошибка в push-токенах, будет список через запятую обрезанных токенов (первые 6 символов)): _(необходимо продумать систему алертов для VALIDATION_ERROR, т.к. это не временная проблема а тут нужны правки в работе системы)_    
   4.4. В случае получения ошибки "INTERNAL_ERROR" (внутренняя ошибка сервиса) повторно отправляем запрос (retry с backoff 5s, 30s, 300s). Если количество попыток превышено, но сообщение не получено провайдером, то процесс отправки уведомления прекращается. В случае, когда событие определено в системе как критически важное, инициируется процесс отправки SMS-сообщения.   
      
5. Push-provider доставляет уведомление пользователю на устройства.   

## Нефункциональные требования 
### Производительность 
1. Система должна поддерживать отправку не менее 5000 сообщений в секунду без снижения производительности.
2. Система должна поддерживать увеличение нагрузки до 15000 сообщений в секунду без снижения производительности.

### Доступность 
1. SLA: Система должна быть доступна 99,95% времени в течение года.
2. Потеря одного инстанса сервиса/Kafka не должна приводить к потере уведомлений.
3. Kafka и Notification Service должны работать в отказоустойчивом режиме.

### Надежность
1. Система должна обеспечивать гарантии доставки согласно типу уведомления.
2. Система должна поддерживать retry-механизмы, описанные в требованиях к ПО.
3. Система должна поддерживать идемпотентность обработки сообщений согласно критичности события в системе.

### Безопасность
1. Device tokens хранятся в зашифрованном виде.
